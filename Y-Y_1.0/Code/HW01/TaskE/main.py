def get_p_n(k, m, apparts_floor):
    """
    Вычисление возможных значений p, n для квартир на этаже

    Args:
        k (unsigned int): номер квартиры
        m (unsigned int): количество этажей
        apparts_floor (unsigned int): количество квартир на этаже

    Returns:
        tuple: номер подъезда и номер этажа
    """
...
def decision_1(k1, m, k2, p2, n2):
    """
    Стандартное решение через циклы

    Args:
        k1, m, k2, p2, n2 (unsigned int): входные данные задачи
    
    Returns:
        tuple: решение задачи, согласно формату вывода
    """
    results_p = set()
    results_n = set()
    apparts_floor = 1
