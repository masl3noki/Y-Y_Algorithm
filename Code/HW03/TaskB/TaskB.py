...
class biGraph():
    ...
    def getShortestPath(self):
        dist = self.getShortestPath_len()
        # Если попасть в end node невозможно, то выводим -1
        if dist == -1:
            return -1
        # Если попасть в bgn node, то выводим сам bgn node (по факту мы не сдвинулись и это единственный короткий путь)
        elif dist == 0:
            return self.__nodes_bgn
        answer = []
        answer.append(str(self.__nodes_end))

        # Пока очередной узел не начальный, прыгаем вдоль короткого пути от end до bgn
        node = self.parents[str(self.__nodes_end)]
        while node != str(self.__nodes_bgn):
            answer.append(node)

            node = self.parents[node]
            # TODO: не уверен, что это работает
            # Если по ключу node ничего нет, то значит и пути нет. Возвращаем -1
            if node == "":
                return -1

        answer.append(str(self.__nodes_bgn))
        # Мы нашли путь от конца в начало. Нужно инвертировать и отформатировать ответ
        answer = answer[::-1]
        return " ".join(answer)
